...
export class AddPostComponent implements OnInit {
  ...
  @ViewChild('postContent') private postContent!: { nativeElement: HTMLElement };
  ...
  public postModel: Post = {
    createdDate: new Date(), description: "", expiryDate: undefined,
  ...
  handleFileInput(event: Event) {
    ...
    if (input && input.files && input.files.length > 0) {
      ...
      // handle the file as needed, e.g., assign to model or upload
  ...
  open(content: HTMLElement): void {
    interface ModalOptions extends NgbModalOptions {
      size: 'lg';
      centered: boolean;
