...
export class BlogPostComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (url.indexOf("preview") > 0) {
      const postModelStr = localStorage.getItem("postModel");
      if (postModelStr) {
        this.postModel = JSON.parse(postModelStr);
        this.startTimer();
      } else {
        // alert("No post model found in localStorage!");
      }
